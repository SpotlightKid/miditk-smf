...
def main(args=None):
    ...
    aa("-v", "--verbose", action="store_true", help="Print debugging info to standard output.")
    aa("midifile", metavar="MIDIFILE", help="Standard MIDI input file.")
    ...
    for i, ev in enumerate(sequence.sysex_events()):
        ...
        if args.checksum:
            ...
            if cs != _ord(ev.data[-2]):
                error(
                    "Checksum verification for msg #%03i failed! " "Expected %i, read %i",
                    i + 1,
                    cs,
                    _ord(ev.data[-2]),
                )
